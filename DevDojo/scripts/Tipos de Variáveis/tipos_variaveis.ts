...
let tipoString1 : string = 'A beautiful message';
let tipoString2 : string = "A beautiful message";
let tipoString3 : string = `${tipoString2} to willian`;
...
// Array:
let listaNumerica : number[] = [1, 2, 3];
let listaNumerica2 : Array<number> = [1, 2, 3];
...
function adicao(valor1: number, valor2: number, valor3?: number): number{
    if (valor3 !== undefined){
...
function concatenacao(valor1: any, valor2: any, valor3?: any): any{
    if (valor3 !== undefined){
        return valor1 + valor2 + valor3;
    }
    return valor1 + valor2;
